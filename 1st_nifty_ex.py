...
def main():    
    ...
    plot_histogram = False #True draws histogram of residuals and data
    ...
    if average_pixels:
        ...
        N = N_pixels
        x = [1/N]
        y = [rms(mat4.flatten(order='C'))/rms(mat1.flatten(order='C'))]  
        ...
        for i in divisors(N):
            ...
            for j, k in itertools.product(range(N//i), range(N//i)):
                ...
                s += 1
            y.append(rms(reduced_mat4.flatten(order='C'))/rms(reduced_mat1.flatten(order='C')))
    ...
    if mode == 0 or mode == 1:
        plot.add(r.adjoint(data), title='Data', cmap = cmap, label='a')
    elif mode == 2 :
